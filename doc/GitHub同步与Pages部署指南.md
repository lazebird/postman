...
> 面向 **Google OAuth（品牌验证 / 隐私政策 URL / 服务条款 URL）** 与扩展商店上架场景的最小化部署指南。
...
3. **敏感数据（Gmail 受限 scope）** 还需通过 Google 的**品牌验证 + 数据使用审核**；
   审核会检查隐私政策是否声明：
   - 收集的数据范围与用途；
   - **遵循 Google API Services User Data Policy（含 Limited Use 限制）**；
   - 数据不会转售、不会用于广告 / 画像 / AI 训练；
   - 数据删除方式与联系方式。
   > 本项目 `docs/privacy/` 已按以上要求撰写（见第 5 节）。
...
**所以：只把 `docs/` 站点发布到 GitHub Pages 就足够满足 Google 授权所需的页面，无需同步源码。**
...
Google 授权只需站点页面，可用 **`sync_mode: rebase` + 独立分支** 或 **只保留 docs 的最小仓库**：
...
A：可以，且这正是推荐做法。Google 只校验「页面能公开访问」，不关心源码是否在 GitHub。
...
A：Google Cloud Console 的「已授权域名」填 `github.io` 即可；
